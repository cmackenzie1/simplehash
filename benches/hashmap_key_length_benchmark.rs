...
fn bench_hashmap_key_lengths(c: &mut Criterion) {
    ...
    for &length in &key_lengths {
        ...
        group.bench_function(BenchmarkId::new("MurmurHash3-32-HashMap-Insert", length), |b| {
            b.iter_custom(|iters| {
                let mut total_duration = std::time::Duration::new(0, 0);
        ...
        group.bench_function(BenchmarkId::new("StdHashMap-Lookup", length), |b| {
            ...
            let lookup_indices: Vec<usize> = (0..1000)
                ...
                .collect();
            let lookup_keys: Vec<&String> = lookup_indices.iter()
                .map(|&i| &keys[i])
                .collect();
        ...
        group.bench_function(BenchmarkId::new("MurmurHash3-32-HashMap-Lookup", length), |b| {
            let mut map: HashMap<String, u32, MurmurHash3BuildHasher> =
                HashMap::with_hasher(MurmurHash3BuildHasher);
...
fn bench_hashmap_fill_factor(c: &mut Criterion) {
    ...
    for &fill_factor in &fill_factors {
        ...
        group.bench_function(BenchmarkId::new("MurmurHash3-32-HashMap", fill_factor), |b| {
            b.iter_custom(|iters| {
                let mut total_duration = std::time::Duration::new(0, 0);
